...
export default function ProductCard(props) {
    ...
    return <>
        ...
        <Card className="text-center product-card img-style ">
            <Card.Header as="h5" className="header"> {props.prod.name} </Card.Header>
            <Link to={`/product/${props.prod.id}`}><Card.Img variant="top" src={props.prod.imageUrl} /></Link>
            <Card.Body>
                <Card.Title> $ {props.prod.price}</Card.Title>

                <div className="buttons-container">
                    <Button
                        onClick={() => addProductToCart(props.prod, 1)}
                        variant="success"
