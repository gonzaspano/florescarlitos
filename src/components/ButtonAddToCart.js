...
export default function ButtonAddToCart({prod}) {
    const { productQuantity, addProductToCart } = useCartContext()

    return <>
        <div className="buyButton-Container">
            <Button variant="dark" onClick={() => addProductToCart(prod, productQuantity)}>
                Agregar al carrito
